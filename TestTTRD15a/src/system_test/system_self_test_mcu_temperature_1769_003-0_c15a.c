/*--------------------------------------------------------------------*-
   ...
   system_self_test_mcu_temperature_1769_003-0_c15a.c (Released 2015-02)
...
// Operating temperature range for LPC1769 MCU
...
-*------------------------------------------------------------------*/
void SYSTEM_SELF_TEST_Check_MCU_temp_Init()
   {
...
/*------------------------------------------------------------------*-
  ...
  SYSTEM_SELF_TEST_Check_MCU_temp()
   ...
   if (Return_value == RETURN_NORMAL)
      ...
      if (++call_count >= 1000)
         ...
         if (Current_temperature_x10_G > (9 * MCU_TEMPERATURE_MAX))
            ...
            SYSTEM_MODE_Change_Mode_Fault(FAULT_MCU_TEMPERATURE);
            }
