/*--------------------------------------------------------------------*-
  ...
  detergent_hatch_1769_003-0_c15a.c (Released 2015-02)
  ...
  In the finished system, this will control the detergent hatch.
...
// Module header
#include "detergent_hatch_1769_003-0_c15a.h"
...
int32_t Detergent_hatch_required_state_G;
int32_t Detergent_hatch_required_state_iG;  // Inverted copy

/*--------------------------------------------------------------------*-

  DETERGENT_HATCH_Init()

  Prepare for DETERGENT_HATCH_Update() task - see below.

-*--------------------------------------------------------------------*/
void DETERGENT_HATCH_Init(void)
   {
   // Placeholder task
   }
...
/*--------------------------------------------------------------------*-
  ...
  DETERGENT_HATCH_Update().

  Placeholder task.

  In finished system, this task will control the "detergent hatch".
  
  This task framework incorporates lightweight Resource Barriers.

-*--------------------------------------------------------------------*/
uint32_t DETERGENT_HATCH_Update(void)
   {
   uint32_t Return_value = RETURN_NORMAL;

   // Check data integrity (lightweight resource barriers)
   if (Detergent_hatch_required_state_G 
       != ~Detergent_hatch_required_state_iG) 
      {
   ...
   if (Return_value == RETURN_NORMAL)
      {
      if (Detergent_hatch_required_state_G == OPEN)
         {
         // Open detergent hatch ...
         }
      else
         {
         // Close detergent hatch ...
         }
      }
